...
- FR-05b **[MVP]**: Structured work-item query against `work_items_current` (not the event log). Filter shapes (combinable, AND semantics):
  ...
  Pagination via `(last_event_seq, work_item_id)` cursor; default page size 100, max 1000. Indexes required to satisfy NFR-perf-1: `(workflow_name, workflow_version, current_state)`, `(claimed_by)`, `(needs_review) WHERE needs_review`. This is the foundation query for agent claim-discovery ("what work is available for me to claim now") and for the federated UI's list views; without it consumers reach into `work_items_current` directly and the substrate's API surface is incomplete.
...
- FR-15 **[MVP]**: Verify actor identity via pluggable verifier before recording any event.
  ...
  - **Storage of canonical bytes:** `payload_canonical_hash` (SHA-256 of the canonical envelope) is persisted on every event row. Re-verification at replay time uses this hash, not jsonb re-serialization, so signature stability survives Postgres version upgrades that change jsonb canonicalization.
...
- BR-12: **API-layer idempotency.** All mutation operations (event append, claim acquire / heartbeat / release, link create / remove, workflow registration) accept a client-supplied idempotency key (UUIDv4). Duplicate keys return the original result deterministically rather than producing a second logical operation. This makes caller-side retry across transient failures (DB connection drop, partial response) safe. For event append, `event_id` doubles as the idempotency key; for other operations, an explicit `idempotency_key` parameter is added.
...
| Duplicate idempotency_key on retry | Caller retries with same `event_id` / `idempotency_key` | Return original result deterministically; no new event row; metric increments | Caller sees idempotent success |
...
- AC-24 [BR-12 / FR-03]: Calling event append twice with the same `event_id` produces exactly one row in `events`. The second call returns the result of the first deterministically. Same property for `idempotency_key` on claim acquire / heartbeat / release / link mutation.
...
- AC-26 [FR-15]: Re-verifying a stored event's signature uses the stored `payload_canonical_hash`, not jsonb re-serialization. A simulated jsonb-formatting change (e.g., key reordering on round-trip) does NOT invalidate previously verified events.
...
- AC-32 [FR-05b]: A query with multiple filters (e.g., `workflow_name=X AND current_state IN (a, b) AND claimable_now=true`) returns exactly the work-items satisfying all filters. Pagination with cursor `(last_event_seq, work_item_id)` returns subsequent pages with no overlap and no gap across concurrent appends to non-matching work-items. Indexes ensure p99 < NFR-perf-1 latency at expected scale.
...
| link create vs link create (same logical link) | serialize; idempotency_key dedups if same logical operation |
