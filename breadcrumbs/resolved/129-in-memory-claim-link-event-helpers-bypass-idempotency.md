...
Already fixed by BC-116 in Session 24. Both `_append_claim_event` and `_append_simple_event` now call `check_idempotency()` at the top and return early if matched. This breadcrumb was filed before the fix was applied.